...
describe('socket.channel: ' + channel, function() {
	...
	before(function() {
		var port = 3700;
		var tmpApp = express();
		var server = tmpApp.listen(port, 'localhost');
		tmpApp.get('/v2/exchange-rates', function(req, res, next) {
			if (mockServersDown) {
	...
	before(function() {
		...
		var tmpApp = express();
		var server = tmpApp.listen(port, 'localhost');
		tmpApp.get('/public', function(req, res, next) {
			if (mockServersDown) {
				return res.status(500).end();
			}
